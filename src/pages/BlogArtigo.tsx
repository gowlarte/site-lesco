...
import heroImg from "@/assets/cop30/hero.webp";
import edificio1 from "@/assets/cop30/edificio-1.webp";
import edificio2 from "@/assets/cop30/edificio-2.webp";
import produtoWpc from "@/assets/cop30/produto-wpc.webp";
import fachadaCop from "@/assets/cop30/fachada-cop30.webp";
import extra1 from "@/assets/cop30/extra-1.webp";
import extra2 from "@/assets/cop30/extra-2.webp";
import related1 from "@/assets/cop30/related-1.webp";
import related2 from "@/assets/cop30/related-2.webp";
...
const ComparisonTable = () => {
  const rows = [
    [t("Umidade e intempéries"), t("permanece estável; madeira incha/racha e exige retrabalhos.")],
    [t("Pragas e fungos"), t("não atrai cupins/mofo; madeira precisa de tratamentos.")],
    [t("Exposição UV"), t("preserva cor/textura; madeira desbota rapidamente.")],
    [t("Manutenção (OPEX)"), t("pede limpeza simples; madeira demanda repintura/verniz.")],
  ];
  return (
...
const CTABlock = () => (
  <section className="rounded-[10px] bg-bone border border-primary/10 px-7 md:px-9 py-7 md:py-8">
    ...
    <p className="mt-4 font-body text-[11px] text-dark">
      {t("Precisa de apoio técnico, amostras ou fichas detalhadas? Nosso time está pronto para apoiar sua especificação.")}
    </p>
  </section>
);
...
const RelatedArticles = () => {
  const items = [
    { img: related1, title: t("Conheça a melhor madeira para brise: beleza e durabilidade") },
    { img: related2, title: t("Onde comprar brise de madeira WPC: Lesco Revestimentos") },
  ];
  return (
...
const BlogArtigo = () => {
  const attributes: AttributeProps[] = [
    { title: t("Estabilidade UV:"), description: t("cor e textura preservadas sob radiação solar intensa.") },
    { title: t("Estabilidade dimensional e umidade:"), description: t("não apodrece, não empena; ideal para a Amazônia chuvosa.") },
    { title: t("Resistência a pragas e fungos:"), description: t("dispensa tratamentos recorrentes contra cupins e mofo.") },
    { title: t("Acabamento premium:"), description: t("fixação oculta, superfície contínua e leitura estética impecável.") },
    { title: t("Baixa manutenção (OPEX):"), description: t("limpeza simples, sem repinturas ou envernizamentos.") },
  ];
  const engineering: AttributeProps[] = [
    { title: t("Estrutura de apoio dimensionada:"), description: t("Vãos usuais (ex.: 60–80 cm, conforme cálculo) para rigidez e planicidade.") },
    { title: t("Modulação inteligente:"), description: t("Paginação de barras e cortes otimizados para reduzir perdas e acelerar montagem.") },
    { title: t("Ventilação do respaldo:"), description: t("Controle de dilatação e microclima, elevando a durabilidade do conjunto.") },
    { title: t("Arremates e encontros resolvidos:"), description: t("Continuidade visual, estanqueidade e proteção à água.") },
    { title: t("Operação simplificada:"), description: t("Rotina de limpeza com água e sabão neutro garante aparência estável.") },
  ];

  return (
    <main className="min-h-screen pt-[100px] pb-[10px] px-[10px]">
      <SEO
        title={t("O Brasil que constrói para o mundo: Lesco e a COP30 — Blog Lesco")}
        description={t("Como a madeira ecológica Lesco assina a fachada do Parque da Cidade, o palco da COP30 em Belém — sustentabilidade, técnica e identidade brasileira.")}
      ...
      <article className="mx-auto w-full max-w-[920px] px-6 sm:px-10 md:px-14 pt-10 md:pt-14 pb-14 md:pb-16">
        ...
        <H1>{t("O Brasil que constrói para o mundo: Lesco e o DNA sustentável que assina a COP30")}</H1>
          ...
          <div className="mt-8 grid grid-cols-12 gap-[10px] items-start">
            <div className="col-span-12 md:col-span-5 flex flex-col gap-7">
              <div className="relative aspect-[4/3] overflow-hidden rounded-[10px]">
                <img src={edificio1} alt={t("Edifício Parque da Cidade ao entardecer")} className="absolute inset-0 w-full h-full object-cover" />
              </div>
              <PullQuote>
                {t("O Brasil tem uma assinatura própria quando o assunto é construção sustentável: unir a estética da natureza à precisão da engenharia.")}
              </PullQuote>
            </div>
            <div className="col-span-12 md:col-span-7">
              <div className="relative aspect-[4/5] overflow-hidden rounded-[10px]">
                <img src={edificio2} alt={t("Detalhe do revestimento WPC à noite")} className="absolute inset-0 w-full h-full object-cover" />
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="mt-14">
          <div className="grid grid-cols-12 gap-[14px] items-start">
            <div className="col-span-4 md:col-span-3">
              <div className="relative aspect-square overflow-hidden rounded-[10px] bg-dark/[0.04]">
                <img src={produtoWpc} alt={`${t("Perfil")} Green Shield 184x20 Red Cedar`} className="absolute inset-0 w-full h-full object-contain p-3" />
              </div>
            </div>
            <div className="col-span-8 md:col-span-9 pt-1">
              <H2>{t("Material de assinatura:")} Green Shield 184x20 {t("em")} Red Cedar</H2>
              <div className="mt-3">
                <Body>
                  {t("O forro externo da fachada utiliza o Lesco Green Shield 184x20 na cor Red Cedar, um Wood Plastic Composite (WPC) que combina fibras naturais e polímeros de alta performance para entregar a leitura calorosa da madeira com desempenho superior em ambiente externo.")}
                </Body>
              </div>
            </div>
          </div>
          ...
          <div className="mt-10 relative aspect-[16/9] overflow-hidden rounded-[10px]">
            <img src={fachadaCop} alt={t("Fachada COP30 com revestimento WPC")} className="absolute inset-0 w-full h-full object-cover" />
          </div>
        ...
        <section className="mt-14">
          ...
          <div className="mt-6">
            <Body>
              {t("O resultado é o que chamamos de luxo funcional: beleza atemporal com desempenho mensurável em uso real.")}
            </Body>
          </div>
        </section>
        ...
        <div className="mt-12 relative aspect-[16/9] overflow-hidden rounded-[10px]">
          <img src={extra1} alt={t("Detalhe do Parque da Cidade")} className="absolute inset-0 w-full h-full object-cover" />
        </div>

        {/* §4 */}
        <section className="mt-14">
          <H2>{t("Escolha de ciclo de vida: WPC Lesco x madeira natural")}</H2>
          <div className="mt-3 mb-5">
            <Body>
              {t("Em clima severo, decisões baseadas apenas no menor custo inicial tendem a cobrar a conta no futuro. Na ótica do ciclo de vida:")}
            </Body>
          </div>
          <ComparisonTable />
        </section>
        ...
        {/* full-width image between sections */}
        <div className="mt-12 relative aspect-[16/9] overflow-hidden rounded-[10px]">
          <img src={extra2} alt={t("Vista do Parque da Cidade ao entardecer")} className="absolute inset-0 w-full h-full object-cover" />
        </div>

        {/* §5 */}
        <section className="mt-14">
          <Body>{t("Branding urbano: a fachada como linguagem de cidade")}</Body>
          <div className="mt-6">
            <H3>{t("Depois do evento, o legado")}</H3>
            <div className="mt-3">
              <Body>
                {t("Em um evento global, a fachada deixa de ser apenas cobertura e se torna mídia. Cada enquadramento, transmissão e visita técnica carrega a narrativa material do lugar. Ao adotar WPC Lesco no Parque da Cidade, Belém comunica ao mundo uma mensagem clara: o futuro da madeira é responsável, técnico e belo.")}
              </Body>
            </div>
          </div>
        </section>
