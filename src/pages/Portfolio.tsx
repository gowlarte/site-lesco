...
const Portfolio = () => {
  return (
    <>
      <SEO
        title={t("Portfólio de Projetos — Lesco")}
        description={t("Cases reais com revestimentos Lesco: residências, projetos comerciais e instalações premium em madeira ecológica.")}
        path="/portfolio"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[10px]">
            {projetos.map((p) => (
              <Link to={`/projetos/${p.slug}`} key={p.slug} className="group block">
                ...
                <p className="font-body text-[11px] font-light uppercase tracking-[0.12em] mt-3 ml-1 text-gray-600">
                  {p.linha}
                </p>
